...
def assert_image_format(image, fcn_name: str, arg_name: str, force_alpha: bool = True):
    ...
    if not len(image.shape) == 3:
        err_msg = (
            ...
            'the "{arg_name}" argument.'.format(
                fcn_name=fcn_name, arg_name=arg_name, n_dim=str(len(image.shape))
            )
        )
        raise TypeError(err_msg)
    ...
    if force_alpha and not image.shape[2] == 4:
        err_msg = (
            ...
            'to the "{arg_name}" argument.'.format(
                fcn_name=fcn_name, arg_name=arg_name, n_layers=str(image.shape[2])
            )
        )
        raise TypeError(err_msg)
    ...
    if not isinstance(opacity, float) and not isinstance(opacity, int):
        err_msg = (
            ...
            '"{fcn_name}".'.format(
                fcn_name=fcn_name,
                arg_name=arg_name,
                var_type=str(type(opacity).__name__),
            )
        )
        raise TypeError(err_msg)
