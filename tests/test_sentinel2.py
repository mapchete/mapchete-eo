...
def test_static_stac_max_cloud_cover(
    sentinel2_stac_mapchete, sentinel2_stac_cloud_cover_mapchete
):
    full_stack = list(sentinel2_stac_mapchete.mp().config.inputs.values())[0]
    filtered = list(sentinel2_stac_cloud_cover_mapchete.mp().config.inputs.values())[0]
    assert len(full_stack.products) > len(filtered.products)
